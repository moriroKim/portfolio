...
const PRIMARY_GROUP_LABEL = "Primary Stack";
      ...
      >
        <ul className="divide-y divide-line/60">
          {dict.groups.map((group) => {
            const isPrimaryGroup = group.label === PRIMARY_GROUP_LABEL;
            return (
              <li
                key={group.label}
                ...
                <ul className="flex flex-wrap gap-1.5">
                  {group.items.map((tech) => (
                    <li
                      ...
                      className={
                        isPrimaryGroup
                          ? "inline-flex items-center gap-1.5 rounded-md border border-violet/45 bg-violet/10 px-2.5 py-1 text-xs font-bold text-violet-deep shadow-[0_1px_0_rgba(124,58,237,0.08)] transition-colors hover:bg-violet/15"
                          : "inline-flex items-center gap-1.5 rounded-md border border-line/80 bg-paper-warm/50 px-2 py-1 text-xs font-medium text-ink/90 transition-colors hover:border-violet/40 hover:bg-paper hover:text-ink"
                      }
                    >
