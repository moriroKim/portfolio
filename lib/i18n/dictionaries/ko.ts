        ...
        caseStudy: {
          ...
          blocks: [
            ...
            },
            {
              ...
              bullets: [
                "깨어날 권한 — 일반 알람은 도즈에서 유지보수 창까지 통째로 미뤄집니다. 도즈 중에도 발화하는 정확 알람 API로 예약하되, 그마저도 약 9분 간격 그리드로 스로틀된다는 것을 전제로 수집 판정을 설계했습니다. 9분 안에 여러 번 깨우려 싸우는 대신, 깨어난 김에 밀린 판정을 몰아서 처리합니다",
                "배터리 최적화 예외 — 예외 목록에 들어가지 않으면 앱이 대기 등급 하위로 내려가 알람과 네트워크가 한 겹 더 조여집니다. 온보딩에서 이 예외를 명시적으로 요청하고, 승인되지 않으면 다음 단계로 넘어가지 못하게 게이트로 묶었습니다",
                "깨어 있는 동안 유지 — 알람이 깨워도 수집과 전송이 끝나기 전에 CPU가 다시 잠들면 그대로 중단됩니다. 작업 구간 동안 부분 웨이크락을 잡되 시한을 걸어 90초 이상 붙들지 않고, 워치독과 다음 알람 틱에서 반납 누락분을 회수합니다. 웨이크락을 오래 쥐는 것은 도즈를 이기는 게 아니라 배터리를 태우는 일입니다",
                "시간 감각 보정 — 도즈에서는 경과 시간 기준 타이머가 늘어납니다. 45초로 예약한 것이 실측에서 136초에서 357초까지 벌어졌습니다. 그래서 타이머를 신뢰하지 않고, 깨어날 때마다 벽시계로 실제 지난 시간을 다시 계산해 밀린 만큼 보정합니다",
                "늦게 도착한 과거 — 늘어진 타이머의 낡은 콜백이 뒤늦게 도착해 이미 새로 시작된 사이클의 네트워크를 반납해 버리는 사고가 있었습니다. 사이클마다 세대 토큰을 부여해, 자기 세대가 아닌 콜백은 무시하도록 했습니다",
                "끊기면 끝나는 체인 — 알람은 반복 예약이 아니라 처리의 마지막 줄에서 다음 알람을 직접 잡는 자기 재예약 체인입니다. 한 고리가 빠지면 영영 침묵하므로 재예약 지점을 코드 한 곳으로 고정하고, 부팅이나 프로세스 종료로 체인이 끊긴 경우의 부활 경로를 따로 뒀습니다",
                "제조사 절전 정책 — 안드로이드 표준 도즈 위에 제조사별 절전 정책이 한 겹 더 얹힙니다. 표준 예외를 받아도 여기서 다시 잠들 수 있어, 단말 제조사 정책까지 확인해 예외 처리를 맞춰야 했습니다",
              ],
            },
            {
              heading: "요금제라는 제약",
