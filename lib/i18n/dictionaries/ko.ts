    ...
    items: [
      {
        ...
        caseStudy: {
          ...
          metrics: [
            ...
            { value: "10.2KB → 1.1KB", label: "위치 배치 요청 본문 · 압축으로 89% 감소", tone: "outcome" },
            { value: "43MB → 6.8MB", label: "단말 1대 월 전송량 · 압축과 연결 재사용 적용 후", tone: "outcome" },
          ...
          blocks: [
            ...
            },
            {
              ...
              steps: [
                { label: "깨어날 권한", text: "일반 알람은 도즈에서 유지보수 창까지 통째로 미뤄집니다. 도즈 중에도 발화하는 정확 알람 API로 예약하되, 그마저도 약 9분 간격 그리드로 스로틀된다는 것을 전제로 수집 판정을 설계했습니다. 9분 안에 여러 번 깨우려 싸우는 대신, 깨어난 김에 밀린 판정을 몰아서 처리합니다" },
                { label: "배터리 최적화 예외", text: "예외 목록에 들어가지 않으면 앱이 대기 등급 하위로 내려가 알람과 네트워크가 한 겹 더 조여집니다. 온보딩에서 이 예외를 명시적으로 요청하고, 승인되지 않으면 다음 단계로 넘어가지 못하게 게이트로 묶었습니다" },
                { label: "깨어 있는 동안 유지", text: "알람이 깨워도 수집과 전송이 끝나기 전에 CPU가 다시 잠들면 그대로 중단됩니다. 작업 구간 동안 부분 웨이크락을 잡되 시한을 걸어 90초 이상 붙들지 않고, 워치독과 다음 알람 틱에서 반납 누락분을 회수합니다. 웨이크락을 오래 쥐는 것은 도즈를 이기는 게 아니라 배터리를 태우는 일입니다" },
                { label: "시간 감각 보정", text: "도즈에서는 경과 시간 기준 타이머가 늘어납니다. 45초로 예약한 것이 실측에서 136초에서 357초까지 벌어졌습니다. 그래서 타이머를 신뢰하지 않고, 깨어날 때마다 벽시계로 실제 지난 시간을 다시 계산해 밀린 만큼 보정합니다" },
                { label: "늦게 도착한 과거", text: "늘어진 타이머의 낡은 콜백이 뒤늦게 도착해 이미 새로 시작된 사이클의 네트워크를 반납해 버리는 사고가 있었습니다. 사이클마다 세대 토큰을 부여해, 자기 세대가 아닌 콜백은 무시하도록 했습니다" },
                { label: "끊기면 끝나는 체인", text: "알람은 반복 예약이 아니라 처리의 마지막 줄에서 다음 알람을 직접 잡는 자기 재예약 체인입니다. 한 고리가 빠지면 영영 침묵하므로 재예약 지점을 코드 한 곳으로 고정하고, 부팅이나 프로세스 종료로 체인이 끊긴 경우의 부활 경로를 따로 뒀습니다" },
                { label: "제조사 절전 정책", text: "안드로이드 표준 도즈 위에 제조사별 절전 정책이 한 겹 더 얹힙니다. 표준 예외를 받아도 여기서 다시 잠들 수 있어, 단말 제조사 정책까지 확인해 예외 처리를 맞춰야 했습니다" },
              ],
              compare: [
                { label: "45초로 예약한 알람이 실제로 발화한 시각", before: "최대 357초", after: "45초 예약", afterRatio: 13 },
              ],
            },
            {
              heading: "요금제라는 제약",
              ...
              compare: [
                { label: "위치 배치 요청 본문 (위치 30건)", before: "10.2KB", after: "1.1KB", afterRatio: 11 },
                { label: "단말 1대 월 전송량", before: "43MB", after: "6.8MB", afterRatio: 16 },
              ],
            },
            {
              heading: "서비스센터에 가야 고칠 수 있는 앱",
              body:
      ...
      },
      {
        ...
        summary:
          "응시 화면부터 채점 관리자, 외부 음성 인식·합성 서비스 연동, 학습 데이터셋 구축까지 담당한 Next.js 풀스택 서비스입니다.",
        ...
        caseStudy: {
          ...
          metrics: [
            ...
            { value: "2종", label: "음성 합성 제공자를 이원화해 한쪽 장애 시 대체" },
          ],
          blocks: [
            {
              heading: "문제",
            ...
            {
              heading: "일회성 도구에서 운영 플랫폼으로",
              body:
                "처음 물려받은 형태는 단순했습니다. 시험을 한 번 치르고, 녹음을 채점하면 끝나는 일회성 도구였습니다. 그런데 실제 운영에 들어가자 요구가 달라졌습니다. 시험은 학교와 학기 단위로 반복되고, 스태프는 관리자 한 명이 아니라 교사·채점자·검수자로 늘어나고, 녹음은 수백 건씩 쌓입니다. 도구를 플랫폼으로 키우는 작업이 이 프로젝트의 실제 몸통이었습니다.",
              steps: [
                { label: "회차 도입", text: "시험을 학교·학기 단위의 회차로 묶고, 회차마다 응시 가능 상태와 차단일, 문항 세트 배정을 뒀습니다. 회차 상태는 스케줄러가 주기적으로 정합을 맞추고, 문항과 타이밍은 응시 시점 리비전으로 고정되어 이후 수정이 과거 회차에 닿지 않습니다" },
                { label: "계정별 권한 시스템", text: "학생과 스태프를 입구에서 나누고, 스태프 화면 30여 개는 DB의 권한 키로 접근을 판정합니다. 역할별 권한에 교사 개인별 예외를 얹을 수 있어, 새 역할이 생겨도 코드 수정 없이 권한 조합으로 대응합니다" },
                { label: "STT 배치 전환", text: "응시 직후 그 자리에서 전사하던 구조를 큐 기반 배치로 옮겼습니다. 상시 워커가 작업을 하나씩 잠가 가져가고 동시 호출 수를 제한해, 응시 피크가 외부 음성 서비스로 그대로 밀려가지 않습니다" },
                { label: "채점 화면 개선", text: "채점자가 수백 개 녹음을 처리하는 화면이라 흐름이 곧 생산성입니다. 녹음을 채점자에게 자동 분배하고, 파형 기반 재생으로 구간을 확인하며, 발음 검수는 별도 화면으로 분리했습니다. 결과는 엑셀로 내보내 학교에 전달합니다" },
              ],
            },
            {
              heading: "검토한 대안",
        ...
        caseStudy: {
          ...
          blocks: [
            ...
            },
            {
              ...
              steps: [
                { label: "접근성 권한 끄기", text: "차단 판정이 접근성 이벤트에 의존하니 권한을 끄면 통째로 무력화됩니다. 별도 포그라운드 서비스를 데드맨 스위치로 두어, 접근성이 꺼지면 유예 후 사용 기록을 1초 주기로 훑어 비허용 앱을 계속 정지시킵니다. 화면 분할·팝업·PIP처럼 앞에 있는 것이 하나가 아닌 경우까지 포함합니다" },
                { label: "브라우저의 암호화 DNS", text: "도메인 차단이 DNS 조회에 기대는데, 크로미엄 계열 브라우저가 암호화된 경로로 조회해 차단을 지나가는 것을 실측했습니다. 개별 도메인 목록과 분리된 전역 차단 목록을 서버에서 내려받도록 하고, 브라우저 앱 자체를 통제 대상에 올렸습니다" },
                { label: "인앱 브라우저", text: "차단된 사이트를 다른 앱 안의 웹뷰로 여는 경로가 있어, 앱 단위가 아니라 화면에 떠 있는 내용 기준으로도 판정하도록 했습니다" },
                { label: "시계 되돌리기", text: "수면 시간 정책이 단말 시각을 기준으로 하니 시각을 바꾸면 풀립니다. 제조사 정책으로 시각 변경 자체를 잠갔습니다" },
                { label: "설정 앱 우회", text: "권한과 앱 관리 화면으로 들어가는 경로를 제조사 정책의 설정 봉쇄로 막았습니다" },
                { label: "앱 강제 종료", text: "죽였다가 다시 켜지는 사이를 노립니다. 로그인 토큰을 네이티브 저장소에 이중으로 두어 재기동 직후에도 정책을 즉시 복원하고, 중요한 원격 명령은 앱 화면 코드를 거치지 않고 네이티브에서 바로 처리해 앱이 죽어 있어도 도착합니다" },
                { label: "패키지 위장과 재설치", text: "이름만 다른 같은 앱들을 하나의 보호 묶음으로 묶어, 개별 대응이 아니라 묶음 단위로 판정합니다" },
              ],
            },
            {
              heading: "결과",
      ...
      },
      {
        ...
        caseStudy: {
          ...
          blocks: [
            ...
            {
              heading: "로그인 하나에서 계정 플랫폼으로",
              body:
                "시작은 로그인 서버였지만, 자사 서비스가 하나씩 합류하면서 요구가 계정 플랫폼으로 자랐습니다. 새 서비스가 들어올 때마다 코드를 고치는 방식으로는 버틸 수 없어, 합류·연동·전파·정리를 각각 구조로 만들었습니다.",
              steps: [
                { label: "서비스 합류는 등록으로", text: "새 제품은 코드 수정이 아니라 등록으로 합류합니다. 클라이언트 식별자와 API 키, 허용 IP, 리다이렉트 주소를 DB에 등록하면 끝이고, CORS 허용 목록도 그 등록에서 동적으로 만들어집니다. 등록이 곧 정책입니다" },
                { label: "가족 연동 원장", text: "보호자와 자녀의 연동 요청·수락·해제를 SSO가 원장으로 관리합니다. 어느 제품에서 연동해도 같은 관계를 모두가 공유하고, 연동 수는 양쪽 모두 상한을 둡니다" },
                { label: "인증 이벤트 전파", text: "연동과 탈퇴 같은 사건은 각 서비스로 웹훅으로 전파됩니다. 멱등키를 붙여 지수 백오프로 세 번 재시도하고, 그래도 실패하면 보관함에 남겨 관리자가 다시 보낼 수 있습니다. 이벤트가 조용히 사라지는 일이 없습니다" },
                { label: "계정 라이프사이클", text: "휴면 전환, 파기 예고와 파기, 본인인증 정보의 보존 기한을 배치가 자동으로 처리합니다. 개인정보 정리가 운영자의 기억이 아니라 스케줄에 걸려 있습니다" },
                { label: "운영 대시보드", text: "회원 조회, 세션 강제 종료, 웹훅 보관함 재처리, 감사 로그까지 별도 대시보드에서 처리합니다. 장애 대응이 DB 콘솔이 아니라 화면에서 끝납니다" },
              ],
            },
            {
              heading: "결과",
