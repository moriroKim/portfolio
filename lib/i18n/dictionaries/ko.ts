        ...
        caseStudy: {
          ...
          blocks: [
            ...
            },
            {
              heading: "결정",
              body:
                "네 가지를 바꿨습니다. 첫째, 여러 곳에서 제각각 하던 데이터 개방 요청을 한 곳으로 모으고, 누가 요청했는지를 세어 마지막 사용자가 끝날 때까지 끄지 않도록 했습니다. 개방된 상태를 최소 6초는 유지하는 규칙도 함께 넣었습니다. 둘째, 절전 모드의 타이머를 믿지 않고 실제 시각을 기준으로 다시 확인하는 장치를 뒀습니다. 셋째, GPS를 켜는 조건을 경과 시간이 아니라 마지막으로 받은 위치가 얼마나 오래됐는지로 바꿨습니다. 넷째, 받은 좌표를 그대로 쓰지 않고 세 단계로 검증했습니다.",
              bullets: [
                "데이터 개방과 반납을 한 곳에서 관리합니다. 요청자별로 세고, 최소 6초 유지하며, 반납이 누락되지 않도록 안전장치를 네 겹 뒀습니다",
                "실제 시각을 기준으로 다시 확인해 절전 모드의 타이머 지연을 보정합니다",
                "GPS 점등 조건을 보유 위치의 나이로 바꿔, 전송에 실패해도 조건이 영구히 잠기지 않게 했습니다",
                "좌표는 정확도 게이트, 위성 개수를 이용한 기지국 좌표 판별, 이동 속도 타당성 순으로 검증합니다",
                "서버 명령은 위치 전송 응답에 실어 내려받고, 즉시성이 필요하면 문자로 앱을 깨웁니다",
              ],
            },
            {
              heading: "결과",
        ...
        caseStudy: {
          ...
          blocks: [
            ...
            },
            {
              heading: "결정",
              body:
                "관통하는 원칙을 하나 정했습니다. 틀릴 바에는 보여주지 않는다는 것입니다. 이 원칙을 네 곳에 똑같이 적용했습니다. 지하철 이동은 역 위치와 거리와 소요시간이 모두 맞아떨어질 때만 표시하고, 하나라도 어긋나면 아무 말도 하지 않습니다. 머문 곳은 일정 반경을 넘어가야 다른 장소로 나누고, 자정을 넘긴 체류는 날짜별로 잘라 계산합니다. 구버전 사용자에게는 해석 화면을 아예 띄우지 않습니다. 실시간 추적은 앱이 판단하지 않고 서버 상태만 따라가게 했습니다.",
              bullets: [
                "지하철 추론은 역 매칭, 거리, 소요시간 세 필터를 모두 통과해야 표시합니다",
                "경로의 공백 판정은 한 곳에서만 계산해, 다른 값으로 그려 선이 겹치는 일을 막았습니다",
                "체류 군집은 시작점에서 반경 3배를 넘어가야 분리하고, 자정을 넘긴 체류는 날짜별로 나눕니다",
                "자녀앱 버전에 따라 화면 기능을 켜고 끄는 게이트를 뒀습니다",
                "실시간 추적은 서버 세션이 진실이고, 푸시는 다음 조회를 앞당기는 신호로만 씁니다",
              ],
            },
            {
              heading: "결과",
        ...
        caseStudy: {
          ...
          blocks: [
            ...
            },
            {
              heading: "결정",
              body:
                "되돌릴 수 없는 것은 건드리지 않는다를 기준으로 삼았습니다. 배포할 때 적용되는 데이터베이스 변경은 추가만 허용하고, 컬럼을 지우거나 데이터를 삭제하는 명령이 하나라도 있으면 배포를 중단시킵니다. 녹음은 재응시를 해도 지우지 않고 세대로 쌓습니다. 문항은 응시 시점의 내용을 따로 보관해서, 나중에 수정해도 이미 치른 회차는 그대로 유지됩니다.",
              bullets: [
                "배포 시 스키마 변경은 추가 전용이며, 파괴적 명령이 감지되면 즉시 배포를 중단합니다",
                "녹음은 삭제 없이 세대로 쌓아, 관리자가 지시한 재응시와 응시자가 스스로 다시 녹음한 것을 구분합니다",
                "회차별 문항과 타이밍을 응시 시점 그대로 고정해 보관합니다",
                "응시 화면의 우회 경로를 막았습니다. 서버 배정과 다른 문항 접근을 차단하고, 번역 기능을 통한 지문 열람을 막고, 마이크 테스트는 해당 로그인 이후 기록만 인정합니다",
              ],
            },
            {
              heading: "음성 파이프라인",
          ...
          blocks: [
            {
              heading: "무엇인가",
              body:
                "일반 전화망 통화에서 한국어와 영어를 양방향으로 실시간 통역하는 오픈소스 시스템입니다. 웹 클라이언트와 임의의 전화번호 사이를 두 개의 동시 음성 세션으로 연결하고, 전송은 통신 API의 미디어 스트림을 사용합니다. 받는 쪽은 앱을 설치할 필요도, 통신사와 연동할 필요도 없습니다. ACL 2026 System Demonstrations 트랙에 채택됐고 MIT 라이선스로 공개되어 있습니다.",
            },
            {
              heading: "풀어야 했던 문제",
              body:
            ...
            },
            {
              heading: "결과",
              body:
                "논문의 핵심 기여인 이중 세션 에코 게이팅은 결정론적 무음 주입과 에너지 기반 음성 활동 감지로 어느 쪽이 말할 차례인지를 판정해, 자기 출력이 되돌아오는 경로를 막습니다. 프로덕션 배포에서 발신자에서 수신자까지 중간 지연 555밀리초, 147통 통화 중 에코 루프 0건, 통화 1분당 0.28달러를 기록했고 한영 통화 155통으로 평가했습니다. 릴레이 서버에는 47개의 테스트 파일을 두어 단위, 컴포넌트, 통합, 부하 시험을 나눠 검증하고 있습니다.",
            },
          ],
        },
      },
